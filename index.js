...
const messageTrigger = {
    activationRegex:
        /\b(send|mail|imagine|generate|make|create|draw|paint|render)\b.*\b(pic|picture|image|drawing|painting|photo|photograph)\b(?:\s+of)?(?:\s+(?:a|an|the|this|that|those)?)?(.+)/i,
    specialCases: {
...
const helpString = [
    `${m(
        "[quiet=false/true] (argument)"
    )} – requests to generate an image and posts it to chat (unless quiet=true argument is specified). Supported arguments: ${m(
        j(Object.values(triggerWords).flat())
    )}.`,
...
const defaultSettings = {
    ...
    prompt_prefix: defaultPrefix ?? "best quality, absurdres, aesthetic,",
    negative_prompt:
        defaultNegative ??
        "lowres, bad anatomy, bad hands, text, error, cropped, worst quality, low quality, normal quality, jpeg artifacts, signature, watermark, username, blurry",
    sampler: "DDIM",
...
function toggleSourceControls() {
    ...
    if (extension_settings.sd.source !== "auto") {
        ...
        onControlNetInput(); //run input to turn off controlnet if not A1111 / SD Forge.
    }
    if (extension_settings.sd.source !== "auto") {
        $("#sd_adetailer_mode").prop("checked", false);
        onAdetailerInput(); //run input to turn off ADetailer if not A1111 / SD Forge.
    }
    //#############################################################
}
...
async function loadSettings() {
    ...
    //hide/show controlnet settings
    extension_settings.sd.controlnet
        ? $("#controlnet_options").show()
        : $("#controlnet_options").hide();
    //----------
    //ADetailer -- @vincedundee
    $("#sd_adetailer_mode").prop("checked", extension_settings.sd.adetailer);
    $("#sd_adetailer_weight")
        .val(extension_settings.sd.adetailer_weight)
        .trigger("input");
    $("input[name='sd_adetailer_models']").prop("checked", function () {
        return extension_settings.sd.adetailer_models.includes($(this).val());
    });
    extension_settings.sd.adetailer
        ? $("#adetailer_options").show()
        : $("#adetailer_options").hide();
    //----------
    //########################################################################
    ...
    $("#sd_clipskip").val(extension_settings.sd.clipskip).trigger("input");
    $("#sd_prompt_prefix")
...
async function onSaveStyleClick() {
    const userInput = await callPopup("Enter style name:", "input", "", {
        okButton: "Save",
    });
...
function onChatChanged() {
    ...
    $("#sd_character_negative_prompt").val(
        key ? extension_settings.sd.character_negative_prompts[key] || "" : ""
    );
}
...
function onClipSkipInput() {
    extension_settings.sd.clipskip = Number($("#sd_clipskip").val());
    $("#sd_clipskip_value").text(extension_settings.sd.clipskip);
    saveSettingsDebounced();
}
...
async function onControlNetInput() {
    ...
    if (
        ...
        toastr.error("ControlNet is only available for A1111 / SD Forge.");
        $("#sd_controlnet_mode").prop("checked", false);
        return;
    }
    //-------------------------------------------
...
//--------------------------------------------------
//ADetailer -- @vincedundee
async function onAdetailerInput() {
    //Don't allow other sources to use ADetailer...cause, you know. It won't work, init
    if (
        !!$("#sd_adetailer_mode").prop("checked") &&
        extension_settings.sd.source !== "auto"
    ) {
        toastr.error("ADetailer is only available for A1111 / SD Forge.");
        $("#sd_adetailer_mode").prop("checked", false);
        return;
    }
    //-------------------------------------------
    extension_settings.sd.adetailer = !!$("#sd_adetailer_mode").prop("checked");
    const optionsDiv = document.getElementById("adetailer_options");
    if (extension_settings.sd.adetailer) {
        toastr.info(
            "ADetailer mode enabled. This will take longer to generate images."
        );
        optionsDiv.style.display = "block";
    } else {
        optionsDiv.style.display = "none";
...
async function onAdetailerModelInput() {
    //console.log('ADetailer Model Input');
    let models = [];
    $("#sd_adetailer_models:checked").each(function () {
        models.push($(this).val());
    });
    //console.log("changing ADetailer Modelssaving...", models);
    extension_settings.sd.adetailer_models = models;
    saveSettingsDebounced();
}
async function onAdetailerWeightInput() {
    extension_settings.sd.adetailer_weight = Number(
        $("#sd_adetailer_weight").val()
    );
    $("#sd_adetailer_weight_value").text(
        extension_settings.sd.adetailer_weight.toFixed(1)
    );
    saveSettingsDebounced();
}
//-----------------------------------------------------
...
async function sendGenerationRequest(
    ...
    callback
        ? callback(prompt, base64Image, generationType)
        : sendMessage(prompt, base64Image, generationType);
    return base64Image;
}
async function generateTogetherAIImage(prompt, negativePrompt) {
    ...
    if (result.ok) {
        ...
        return {
            format: "jpg",
            data: data?.output?.choices?.[0]?.image_base64,
        };
    } else {
 ...
 */
async function addAlwaysOnScripts() {
    console.log("addAlwaysOnScripts");
    ...
    if (extension_settings.sd.adetailer) {
        console.log("It's true! mammy, mammy! They made ADetailer truuueeeeeee! Put your make up on!");
        ao_scripts.alwayson_scripts.ADetailer = {
            args: extension_settings.sd.adetailer_models.map((model) => ({
                ad_model: model,
                ad_confidence: extension_settings.sd.adetailer_weight ?? 0.3,
            })),
        };
    }
    // Check if ControlNet is enabled for the current prompt type
    if (
        ...
        try {
            switch (extension_settings.sd.controlnet_inputRef) {
                ...
                case "user":
                    ...
                    if (!srcValue) {
                        srcValue = $("div[is_user='true'] .avatar img").attr(
                            "src"
                        );
        ...
        ao_scripts.alwayson_scripts.ControlNet = {
            args: [
                {
                    enabled: true,
                    image: base64Data, //recently fuckin changed without notice to 'image'
...
async function generateAutoImage(prompt, negativePrompt) {
    console.log("generateAutoImage", getSdRequestBody());
    const postBody = {
        ...getSdRequestBody(),
        prompt: prompt,
        ...
        steps: extension_settings.sd.steps,
        override_settings: {
            CLIP_stop_at_last_layers: extension_settings.sd.clipskip ?? 1,
            //sd_vae: "sd-vae-ft-mse.safetensors"
            //sd_vae: 'NAI_animefull-final.vae.pt',
        },
    ...
    //ControlNet add -- @vincedundee
    //post request moved to accomodate controlNet
    const controlNet = (await addAlwaysOnScripts()) || {};
...
async function generateComfyImage(prompt, negativePrompt) {
    ...
    let workflow = (await workflowResponse.json()).replace(
        '"%prompt%"',
    ...
    workflow = workflow.replace(
        '"%seed%"',
        JSON.stringify(Math.round(Math.random() * Number.MAX_SAFE_INTEGER))
    );
...
async function onComfyOpenWorkflowEditorClick() {
    ...
    const popupResult = callPopup(editorHtml, "confirm", undefined, {
        okButton: "Save",
        wide: true,
        large: true,
        rows: 1,
...
jQuery(async () => {
    ...
    registerSlashCommand(
        ...
        changeComfyWorkflow,
        ["icw"],
        "(workflowName) - change the workflow to be used for image generation with ComfyUI, e.g. <tt>/imagine-comfy-workflow MyWorkflow</tt>"
    );
    ...
    //-----------------------------------
    //ADetailer @vincedundee ------------------------
    $("#sd_adetailer_mode").on("input", onAdetailerInput);
    $("#sd_adetailer_weight").on("input", onAdetailerWeightInput);
    $("input[name='sd_adetailer_models']").on("change", onAdetailerModelInput);
    //-----------------------------------
    $("#sd_source").on("change", onSourceChange);
    $("#sd_scale").on("input", onScaleInput);
    $("#sd_steps").on("input", onStepsInput);
    $("#sd_clipskip").on("input", onClipSkipInput);
